...
//        $this->db->join('t_content cntn', 'cntn.menu_alias=menu.menu_alias','LEFT');
    ...
    function getHeaderMenu ($parent = 0, $ctgr = 'left', $type='backend') {
        $this->db->select("menu.id, menu.menu_name, menu.menu_name_en,menu.menu_link, 
            menu.menu_web_link, menu.menu_parent, menu.menu_ctgr, menu.menu_order,menu.menu_alias,menu.menu_desc,menu.menu_image,
...
//        }
    ...
    function getLeftMenu ($parent = 0, $ctgr = 'left', $type='backend', $order = '') {
...
//    }
    ...
    function getMenus ($parent = 0) {
        $this->db->select("menu.id, menu.menu_name,menu.menu_link, menu.menu_icon, "
                . "menu.menu_parent, menu.menu_order,menu.menu_alias,menu.menu_code");
        $this->db->from("$this->_table as menu");
...
//            $this->db->where("menu.menu_ctgr = ", $ctgr);
//        }
        $this->db->order_by('menu.menu_parent asc');
        $this->db->order_by('menu.menu_order asc');
...
//            echo $this->db->last_query();exit;
    ...
    function getMenuCategoryData($limit = 10, $offset = 0, $where = array(), $condition = 'like', $order = 'a.id,ASC') {
        
        $this->db->distinct();
        $this->db->select("a.id,a.menu_name");
        $this->db->from("$this->_table as a");
        $this->db->join('m_category ctgr','on ctgr.ctgr_menu_id=a.id');
        if ($order != '') {
            $order = explode(',', $order);
            if (sizeof($order) > 1) {
        ...
        if ($where && sizeof($where) > 0) {
            foreach ($where as $key => $value) {
                if ($value == '') {
                    ...
                    $this->db->$condition($key, $value, 'after','both');
                }
            }
        }
...
//            echo $this->db->last_query();exit;
    ...
    function getLeftforFeedbackMenu($parent = 0, $ctgr = 'left', $type='backend', $order = '') {
...
//        $this->db->join('t_content cntn', 'cntn.menu_alias=menu.menu_alias','LEFT');
        ...
        $this->db->where("menu.menu_active = ", 'Y');
        $this->db->where("menu.menu_type in($type) ");
        $this->db->where("menu.menu_code is not null ");
...
//        }
    ...
    function getLeftforGreenMenu($parent = 0, $ctgr = 'left', $type='backend', $order = '') {
...
//        $this->db->join('t_content cntn', 'cntn.menu_alias=menu.menu_alias','LEFT');
        ...
        $this->db->where("menu.menu_active = ", 'Y');
        $this->db->where("menu.menu_type in($type) ");
        $this->db->where("menu.menu_code =",'default');
        $this->db->or_where("menu.id =",'32');
